...
export async function handlerUploadThumbnail(cfg: ApiConfig, req: BunRequest) {
  ...
  const ext = thumbnailData.type.split("/")[1];
  const filePath = path.join(cfg.assetsRoot, `${video.id}.${ext}`);

  await Bun.write(filePath, arrayBuffer);

  video.thumbnailURL = `http://localhost:${cfg.port}/assets/${video.id}.${ext}`;
